...
class PccDownloader {
     ...
     */
    async searchAndWait(page, keyword, chapter) {
            ...
            if (await select.count() > 0) {
                const selected = await select.selectOption(chapter).catch(() => null)
                    ...
                    ?? await page.evaluate((ch) => {
                        const sel = document.querySelector("#tecCode");
                        if (!sel)
                            return false;
                        const opt = Array.from(sel.options).find(o => o.value.startsWith(ch) || o.text.startsWith(ch));
                        if (!opt)
                            return false;
                        sel.value = opt.value;
    ...
    async listChapters() {
        ...
        try {
            ...
            const chapters = await page.evaluate(() => {
                const select = document.querySelector("#tecCode");
                if (!select)
                    return [];
        ...
        try {
            ...
            const rows = await page.evaluate(() => {
                ...
                return trs.map((tr, index) => {
                    ...
                    const btnTexts = allBtns.map(b => b.innerText.trim().toUpperCase());
                    return {
                        index,
                        code: tds[1]?.innerText?.trim() || "",
                        name: tds[2]?.innerText?.trim() || "",
                        fullVersion: tds[3]?.innerText?.replace(/DOCX版|DOC版|ODT版|XLS版|ODS版|PDF版/g, "").trim() || "",
                        detailVersion: tds[4]?.innerText?.replace(/DOCX版|DOC版|ODT版|XLS版|ODS版|PDF版/g, "").trim() || "",
